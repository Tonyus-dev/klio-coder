...
const content = `import { Habit, DailyLog } from '../types';
...
export default function TodayDashboard({ 
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-3xl mx-auto font-sans text-[#F7EFE7]">
      ...
      <div className="space-y-4">
        {[
          ...
          return (
            <div key={section.id} className="bg-[#0B0D12] rounded-2xl border border-[#252936] overflow-hidden shadow-lg">
              <button 
              ...
              {!isCollapsed && (
                <div className="p-3 sm:p-4 space-y-3">
                  {sectionHabits.map(habit => {
                    const progressVal = dailyLog.habits[habit.id] || 0;
                    ...
                    return (
                      <div key={habit.id} className={\`group flex items-center justify-between p-3 sm:p-4 rounded-xl border transition-all \${isFullyCompleted ? 'bg-[#10131A] border-[#252936] opacity-60' : 'bg-[#0B0D12] border-[#252936] hover:border-[#FF4C1F]/30'}\`}>
                        ...
                        <div className="flex items-center gap-1 bg-[#10131A] border border-[#252936] p-1 rounded-xl shrink-0 ml-2">
                          <button
                            onClick={() => onLogHabit(habit.id, Math.max(0, progressVal - 1))}
                            disabled={progressVal === 0}
                            className={\`p-1 sm:p-1.5 rounded-lg transition-colors \${style.btnBg}\`}
                          >
                            <Minus className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                          </button>
                          
                          <span className="text-[10px] sm:text-[11px] font-mono font-bold text-[#F7EFE7] px-1 text-center min-w-[2rem]">
                            {progressVal}
                          </span>
...
fs.writeFileSync('src/components/TodayDashboard.tsx', content);
