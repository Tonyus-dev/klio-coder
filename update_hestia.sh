#!/bin/bash
mkdir -p hestia-console/chama

# Refatorando server.js para hestia.js e separando a Chama
cat << 'CHAMA' > hestia-console/chama/presence.js
const fs = require('fs');
...
cat << 'SRV' > hestia-console/hestia.js
...
# Update package.json to point to hestia.js
sed -i 's/server.js/hestia.js/g' hestia-console/package.json
sed -i 's/server.js/hestia.js/g' hestia-console/packaging/hestia-console.service
sed -i 's/server.js/hestia.js/g' hestia-console/scripts/build-deb.sh
rm -f hestia-console/server.js

# Update UI to explicitly show Chama
